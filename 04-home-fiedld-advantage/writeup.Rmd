...
for(i in seq_along(Win_game)){
  Win_game[i] <- rbinom(7, 1, pb)
  if (Win_game[i]>= 4){
    j = j + 1
  }
}

p_hat_without<- mean(Win_game)
p_hat_without

```
The probability without home field advantage that Braves win the world series by simulation is 0.551.
...
p_hat_with
```
The probability with home field advantage that Braves win the world series by simulation is 0.602.
