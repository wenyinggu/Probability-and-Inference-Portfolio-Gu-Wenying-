...
estimate.ci <- function(data, mod, par.int, R = 10, smoo = 0.3) {
  N<- length(data)
  sum.measure <- get(paste0("par.int1",dist1))
...
cover <- NA
for (sims in 1:10) {
  cover[sims] <- generate_data (N, dist1,1.4,3)%>% estimate.ci(mod = model1, par.int = par.int1, R=10,smoo = 0.3) %>% capture_par(true.par = true.par1)
}
