...
Quantile 55% has more precision,quantiles 5% and 95% have less precision.
```{r}
...
Quantile 5% has more precision,quantile 95% has less precision.
```{r}
quants<-seq(0.05, 0.95, 0.05)
Sum_quantile <- data.frame(matrix(NA, nrow = 5000, ncol = length(quants)))
for(j in 1:5000){
...
The highest density is 0.95 for Median has the tightest sampling distribution. 
```{r}
quants<-seq(0.05, 0.95, 0.05)
Sum_quantile <- data.frame(matrix(NA,nrow = 5000, ncol = length(quants)))
for(j in 1:5000){
...
Here is the Mixture Distribution Graph I
```{r}
rf3 <- function(N){
...
Here is the Density Graph for Mixture Distribution I
```{r}
rf3 <- function(N){
  G <- sample(0:2, N, replace = TRUE, prob = c(5,3,2))
  (G==0)*rnorm(N) + (G==1)*rnorm(N,4) + (G==2)*rnorm(N,-4,2)
...
Here is the Mixture Distribution Graph II
```{r}
rf4 <- function(N){
...
Here is the Density Graph for Mixture Distribution II
```{r}
rf4 <- function(N){
  G <- sample(0:1, N, replace = TRUE)
  (G==0)*rbeta(N,5,1) + (G==1)*rbeta(N,1,5)
